...
while line and True:	
	...
	try:
        ...
        '''
	#variant_list = PreAnnotationDB.find_one({'chrom':CUR_CHROM,'beg':str(begpos), 'end':str(endpos)})
	variant_list = PreAnnotationDB.find_one({'chrom':CUR_CHROM,'beg':str(begpos), 'end':str(endpos), 'type': vartype, 'ref':ref, 'alt':var})        
			...
			line = infile.readline()
        		...
        		for v,val in variant_list.items():
                        	...
                        	if (v == 'annot'):
                                	val = val.encode('ascii', 'ignore')
					'''
					For the cases when annot starts with @@
					'''
					...
					if val.startswith("@@"):
						val = val[2:]
                                	...
                                	annot_res = val.split("@@")
                                	s = len(annot_res) 
					#make sure the annotation has 76 columns
					if s == 76: 
						#extract only  the category 1 and 2
						...
						junk.write(str(found_var) + "\t" + CUR_CHROM + "\t" +str(begpos) + "\t" + str(endpos) + "\t" + str(vartype) + "\t" + str(ref) + "\t" + str(var)+ "\t" + str(AN) + "\t" + str(AC) + "\t" + str(HWE) + "\t" + str(gs) + "\t")                                            
                                                junk.write("\t".join(annot_res[0:s]))
                                                junk.write("\n")   
        		outfile.flush()
        		line = infile.readline()
        		continue   
junk.write(CUR_CHROM + " " + str(counterVar) + " variants")        
